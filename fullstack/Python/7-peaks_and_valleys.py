...
def peaks_and_valleys(data, options="None") -> list:
    def peaks(data) -> list:
        ...
        for item in range(1, len(data) - 1):
            if (
                data[item] > data[item - 1] and data[item] > data[item + 1]
            ):  # If item is greater than previous item, and greater than following item, then is a peak.
    ...
    def valleys(data) -> list:
        ...
        for item in range(1, len(data) - 1):
            if (
                data[item] < data[item - 1] and data[item] < data[item + 1]
            ):  # If item is less then previous item, and less than following item, then is a valley.
...
if __name__ == "__main__":
    ...
    for i in reversed(range(0, max(test_data))):
        ...
        for item in range(0, len(test_data)):
            ...
            if (
                test_data[item] >= highest_peak
            ):  # Get highest peak to use later for water bounds.
            ...
            elif test_data[item] >= highest_peak or i > test_data[item] + (
                highest_peak - test_data[item] - 1
            ):  # or test_data[item] == test_data[len(test_data)-1]: # I don't know, I brute forced this into existence. Become air.
